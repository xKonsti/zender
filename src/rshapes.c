 ...
 *
 *   ADDITIONAL NOTES:
 *       Shapes can be draw using 3 types of primitives: LINES, TRIANGLES and
 * QUADS Some functions implement two drawing options: TRIANGLES and QUADS, by
 * default TRIANGLES are used but QUADS implementation can be selected with
 * SUPPORT_QUADS_DRAW_MODE define
 *
 *       Some functions define texture coordinates (rlTexCoord2f()) for the
 * shapes and use a user-provided texture with SetShapesTexture(), the pourpouse
 * of this implementation is allowing to reduce draw calls when combined with a
 * texture-atlas
 *
 *       By default, raylib sets the default texture and rectangle at
 * InitWindow()[rcore] to one white character of default font [rtext], this way,
 * raylib text and shapes can be draw with a single draw call and it also allows
 * users to configure it the same way with their own fonts
 *
 *   CONFIGURATION:
 ...
 * Lines-based shapes still use LINES
 *
 *
 *   LICENSE: zlib/libpng
 *
...
//----------------------------------------------------------------------------------
static Texture2D texShapes = {
    1, 1, 1, 1,
    7}; // Texture used on shapes drawing (white pixel loaded by rlgl)
static Rectangle texShapesRec = {
    0.0f, 0.0f, 1.0f, 1.0f}; // Texture source rectangle used on shapes drawing
...
Texture2D GetShapesTexture(void) { return texShapes; }

// Get texture source rectangle that is used for shapes drawing
Rectangle GetShapesTextureRectangle(void) { return texShapesRec; }
...
void DrawLineBezier(Vector2 startPos, Vector2 endPos, float thick,
  ...
  for (int i = 1; i <= SPLINE_SEGMENT_DIVISIONS; i++) {
    ...
    current.y = EaseCubicInOut((float)i, startPos.y, endPos.y - startPos.y,
                               (float)SPLINE_SEGMENT_DIVISIONS);
    current.x =
        previous.x + (endPos.x - startPos.x) / (float)SPLINE_SEGMENT_DIVISIONS;
...
// Draw a dashed line
void DrawLineDashed(Vector2 startPos, Vector2 endPos, int dashSize,
  ...
  // If the line is too short for dashing or dash size is invalid, draw a solid
  // thick line
  ...
  while (distanceTraveled < lineLength) {
    ...
    Vector2 dashEndPos = {startPos.x + dashEndDist * dirX,
                          startPos.y + dashEndDist * dirY};

    // Draw the dash segment
    rlVertex2f(currentPos.x, currentPos.y);
    rlVertex2f(dashEndPos.x, dashEndPos.y);

    // Update the distance traveled and move the current position for the next
    // dash
...
void DrawCircleSector(Vector2 center, float radius, float startAngle,
  ...
  for (int i = 0; i < segments / 2; i++) {
    ...
    rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                 shapeRect.y / texShapes.height);
    rlVertex2f(center.x + cosf(DEG2RAD * (angle + stepLength * 2.0f)) * radius,
               center.y + sinf(DEG2RAD * (angle + stepLength * 2.0f)) * radius);
...
void DrawCircleGradient(int centerX, int centerY, float radius, Color inner,
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f((float)centerX + cosf(DEG2RAD * (i + 10)) * radius,
               (float)centerY + sinf(DEG2RAD * (i + 10)) * radius);
    rlColor4ub(outer.r, outer.g, outer.b, outer.a);
    rlVertex2f((float)centerX + cosf(DEG2RAD * i) * radius,
               (float)centerY + sinf(DEG2RAD * i) * radius);
  }
  rlEnd();
}
...
void DrawEllipse(int centerX, int centerY, float radiusH, float radiusV,
                 Color color) {
  DrawEllipseV((Vector2){(float)centerX, (float)centerY}, radiusH, radiusV,
               color);
}
...
void DrawEllipseV(Vector2 center, float radiusH, float radiusV, Color color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    rlColor4ub(color.r, color.g, color.b, color.a);
    rlVertex2f(center.x, center.y);
    rlVertex2f(center.x + cosf(DEG2RAD * (i + 10)) * radiusH,
               center.y + sinf(DEG2RAD * (i + 10)) * radiusV);
    rlVertex2f(center.x + cosf(DEG2RAD * i) * radiusH,
               center.y + sinf(DEG2RAD * i) * radiusV);
  }
  rlEnd();
}
...
void DrawEllipseLines(int centerX, int centerY, float radiusH, float radiusV,
                      Color color) {
  DrawEllipseLinesV((Vector2){(float)centerX, (float)centerY}, radiusH, radiusV,
                    color);
}

// Draw ellipse outline
void DrawEllipseLinesV(Vector2 center, float radiusH, float radiusV,
                       Color color) {
  rlBegin(RL_LINES);
  for (int i = 0; i < 360; i += 10) {
    rlColor4ub(color.r, color.g, color.b, color.a);
...
void DrawRing(Vector2 center, float innerRadius, float outerRadius,
  ...
  if (segments < minSegments) {
    ...
    // (usually 0.5f)
    float th =
        acosf(2 * powf(1 - SMOOTH_CIRCLE_ERROR_RATE / outerRadius, 2) - 1);
    segments = (int)((endAngle - startAngle) * ceilf(2 * PI / th) / 360);
  ...
  // Not a ring
  if (innerRadius <= 0.0f) {
    DrawCircleSector(center, outerRadius, startAngle, endAngle, segments,
                     color);
    return;
  }
  ...
  for (int i = 0; i < segments; i++) {
    ...
    rlTexCoord2f(shapeRect.x / texShapes.width,
                 (shapeRect.y + shapeRect.height) / texShapes.height);
    rlVertex2f(center.x + cosf(DEG2RAD * angle) * outerRadius,
               center.y + sinf(DEG2RAD * angle) * outerRadius);

    rlTexCoord2f(shapeRect.x / texShapes.width, shapeRect.y / texShapes.height);
    rlVertex2f(center.x + cosf(DEG2RAD * angle) * innerRadius,
               center.y + sinf(DEG2RAD * angle) * innerRadius);

    rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                 shapeRect.y / texShapes.height);
    rlVertex2f(center.x + cosf(DEG2RAD * (angle + stepLength)) * innerRadius,
               center.y + sinf(DEG2RAD * (angle + stepLength)) * innerRadius);

    rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                 (shapeRect.y + shapeRect.height) / texShapes.height);
    rlVertex2f(center.x + cosf(DEG2RAD * (angle + stepLength)) * outerRadius,
               center.y + sinf(DEG2RAD * (angle + stepLength)) * outerRadius);
...
void DrawRingLines(Vector2 center, float innerRadius, float outerRadius,
  ...
  if (innerRadius <= 0.0f) {
    DrawCircleSectorLines(center, outerRadius, startAngle, endAngle, segments,
                          color);
    return;
  }
...
void DrawRectangle(int posX, int posY, int width, int height, Color color) {
  DrawRectangleV((Vector2){(float)posX, (float)posY},
                 (Vector2){(float)width, (float)height}, color);
}

// Draw a color-filled rectangle (Vector version)
// NOTE: On OpenGL 3.3 and ES2 we use QUADS to avoid drawing order issues
void DrawRectangleV(Vector2 position, Vector2 size, Color color) {
  DrawRectanglePro((Rectangle){position.x, position.y, size.x, size.y},
...
void DrawRectangleGradientV(int posX, int posY, int width, int height,
  ...
  DrawRectangleGradientEx(
      (Rectangle){(float)posX, (float)posY, (float)width, (float)height}, top,
      bottom, bottom, top);
}
...
void DrawRectangleGradientH(int posX, int posY, int width, int height,
  ...
  DrawRectangleGradientEx(
      (Rectangle){(float)posX, (float)posY, (float)width, (float)height}, left,
      left, right, right);
}
...
void DrawRectangleLines(int posX, int posY, int width, int height,
  ...
  rlVertex2f((float)posX + (float)width - xOffset, (float)posY + yOffset);
  rlVertex2f((float)posX + (float)width - xOffset,
             (float)posY + (float)height - yOffset);
  ...
  /*
  // Previous implementation, it has issues... but it does not require view
  matrix... #if defined(SUPPORT_QUADS_DRAW_MODE) DrawRectangle(posX, posY,
  width, 1, color); DrawRectangle(posX + width - 1, posY + 1, 1, height - 2,
  color); DrawRectangle(posX, posY + height - 1, width, 1, color);
...
void DrawRectangleLinesEx(Rectangle rec, float lineThick, Color color) {
  ...
  Rectangle bottom = {rec.x, rec.y - lineThick + rec.height, rec.width,
                      lineThick};
  Rectangle left = {rec.x, rec.y + lineThick, lineThick,
                    rec.height - lineThick * 2.0f};
  Rectangle right = {rec.x - lineThick + rec.width, rec.y + lineThick,
                     lineThick, rec.height - lineThick * 2.0f};
...
// Draw rectangle with rounded edges
// TODO: This function should be refactored to use RL_LINES, for consistency
// with other Draw*Lines()
...
void DrawRectangleRoundedLinesEx(Rectangle rec, float roundness, int segments,
  ...
  // Not a rounded rectangle
  if (roundness <= 0.0f) {
    DrawRectangleLinesEx((Rectangle){rec.x - lineThick, rec.y - lineThick,
  ...
  */
  const Vector2 point[16] = {
      ...
      {(float)(rec.x + rec.width) - innerRadius - 0.5f,
       rec.y - lineThick + 0.5f},
      {rec.x + rec.width + lineThick - 0.5f,
       (float)rec.y + innerRadius + 0.5f}, // PO, P1, P2
      {rec.x + rec.width + lineThick - 0.5f,
       (float)(rec.y + rec.height) - innerRadius - 0.5f},
      {(float)(rec.x + rec.width) - innerRadius - 0.5f,
       rec.y + rec.height + lineThick - 0.5f}, // P3, P4
      {(float)rec.x + innerRadius + 0.5f,
       rec.y + rec.height + lineThick - 0.5f},
      {rec.x - lineThick + 0.5f,
       (float)(rec.y + rec.height) - innerRadius - 0.5f},
      {rec.x - lineThick + 0.5f,
       (float)rec.y + innerRadius + 0.5f}, // P5, P6, P7
      ...
      {rec.x + rec.width - 0.5f,
       (float)(rec.y + rec.height) - innerRadius - 0.5f}, // P10, P11
      {(float)(rec.x + rec.width) - innerRadius - 0.5f,
       rec.y + rec.height - 0.5f},
      {(float)rec.x + innerRadius + 0.5f,
       rec.y + rec.height - 0.5f}, // P12, P13
  ...
  const Vector2 centers[4] = {
      ...
      {(float)(rec.x + rec.width) - innerRadius - 0.5f,
       (float)rec.y + innerRadius + 0.5f}, // P16, P17
      {(float)(rec.x + rec.width) - innerRadius - 0.5f,
       (float)(rec.y + rec.height) - innerRadius - 0.5f},
      {(float)rec.x + innerRadius + 0.5f,
       (float)(rec.y + rec.height) - innerRadius - 0.5f} // P18, P19
  };
  ...
  if (lineThick > 1) {
    ...
    // Draw all the 4 corners first: Upper Left Corner, Upper Right Corner,
         ...
         ++k) // Hope the compiler is smart enough to unroll this loop
    {
      ...
      for (int i = 0; i < segments; i++) {
        ...
        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * innerRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * innerRadius);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * outerRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * outerRadius);
    ...
    // Draw all of the 4 corners first: Upper Left Corner, Upper Right Corner,
    // Lower Right Corner, Lower Left Corner
    for (int k = 0; k < 4;
         ++k) // Hope the compiler is smart enough to unroll this loop
    {
...
void DrawPoly(Vector2 center, int sides, float radius, float rotation,
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlTexCoord2f(shapeRect.x / texShapes.width,
                 (shapeRect.y + shapeRect.height) / texShapes.height);
    rlVertex2f(center.x + cosf(centralAngle) * radius,
               center.y + sinf(centralAngle) * radius);

    rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                 shapeRect.y / texShapes.height);
    rlVertex2f(center.x + cosf(nextAngle) * radius,
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlVertex2f(center.x, center.y);
    rlVertex2f(center.x + cosf(centralAngle + angleStep) * radius,
               center.y + sinf(centralAngle + angleStep) * radius);
    rlVertex2f(center.x + cosf(centralAngle) * radius,
               center.y + sinf(centralAngle) * radius);
...
void DrawPolyLinesEx(Vector2 center, int sides, float radius, float rotation,
  ...
  float innerRadius =
      radius - (lineThick * cosf(DEG2RAD * exteriorAngle / 2.0f));
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlTexCoord2f(shapeRect.x / texShapes.width, shapeRect.y / texShapes.height);
    rlVertex2f(center.x + cosf(centralAngle) * innerRadius,
               center.y + sinf(centralAngle) * innerRadius);

    rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                 (shapeRect.y + shapeRect.height) / texShapes.height);
    rlVertex2f(center.x + cosf(nextAngle) * innerRadius,
               center.y + sinf(nextAngle) * innerRadius);
...
void DrawSplineBasis(const Vector2 *points, int pointCount, float thick,
  ...
  for (int i = 0; i < (pointCount - 3); i++) {
    float t = 0.0f;
    Vector2 p1 = points[i], p2 = points[i + 1], p3 = points[i + 2],
            p4 = points[i + 3];
...
void DrawSplineBezierQuadratic(const Vector2 *points, int pointCount,
  ...
  if (pointCount >= 3) {
    for (int i = 0; i < pointCount - 2; i += 2)
      DrawSplineSegmentBezierQuadratic(points[i], points[i + 1], points[i + 2],
                                       thick, color);

    // Cap circle drawing at the end of every segment
    // for (int i = 2; i < pointCount - 2; i += 2) DrawCircleV(points[i],
    // thick/2.0f, color);
  }
}
...
void DrawSplineBezierCubic(const Vector2 *points, int pointCount, float thick,
  ...
  if (pointCount >= 4) {
    for (int i = 0; i < pointCount - 3; i += 3)
      DrawSplineSegmentBezierCubic(points[i], points[i + 1], points[i + 2],
                                   points[i + 3], thick, color);

    // Cap circle drawing at the end of every segment
    // for (int i = 3; i < pointCount - 3; i += 3) DrawCircleV(points[i],
    // thick/2.0f, color);
  }
}
...
void DrawSplineSegmentBezierQuadratic(Vector2 p1, Vector2 c2, Vector2 p3,
  ...
  for (int i = 1; i <= SPLINE_SEGMENT_DIVISIONS; i++) {
    ...
    // NOTE: The easing functions aren't suitable here because they don't take a
    // control point
...
Vector2 GetSplinePointBasis(Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4,
                            float t) {
  Vector2 point = {0};
...
Vector2 GetSplinePointCatmullRom(Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4,
                                 float t) {
  Vector2 point = {0};
...
Vector2 GetSplinePointBezierQuad(Vector2 startPos, Vector2 controlPos,
                                 Vector2 endPos, float t) {
  Vector2 point = {0};
...
Vector2 GetSplinePointBezierCubic(Vector2 startPos, Vector2 startControlPos,
                                  Vector2 endControlPos, Vector2 endPos,
                                  float t) {
  Vector2 point = {0};
  ...
  point.y = a * startPos.y + b * startControlPos.y + c * endControlPos.y +
            d * endPos.y;
  point.x = a * startPos.x + b * startControlPos.x + c * endControlPos.x +
            d * endPos.x;
...
bool CheckCollisionLines(Vector2 startPos1, Vector2 endPos1, Vector2 startPos2,
  ...
  float div = (endPos2.y - startPos2.y) * (endPos1.x - startPos1.x) -
              (endPos2.x - startPos2.x) * (endPos1.y - startPos1.y);

  if (fabsf(div) >= FLT_EPSILON) {
    collision = true;

    float xi = ((startPos2.x - endPos2.x) *
                    (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
                (startPos1.x - endPos1.x) *
                    (startPos2.x * endPos2.y - startPos2.y * endPos2.x)) /
               div;
    float yi = ((startPos2.y - endPos2.y) *
                    (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
                (startPos1.y - endPos1.y) *
                    (startPos2.x * endPos2.y - startPos2.y * endPos2.x)) /
               div;

    if (((fabsf(startPos1.x - endPos1.x) > FLT_EPSILON) &&
         (xi < fminf(startPos1.x, endPos1.x) ||
          (xi > fmaxf(startPos1.x, endPos1.x)))) ||
        ((fabsf(startPos2.x - endPos2.x) > FLT_EPSILON) &&
         (xi < fminf(startPos2.x, endPos2.x) ||
          (xi > fmaxf(startPos2.x, endPos2.x)))) ||
        ((fabsf(startPos1.y - endPos1.y) > FLT_EPSILON) &&
         (yi < fminf(startPos1.y, endPos1.y) ||
          (yi > fmaxf(startPos1.y, endPos1.y)))) ||
        ((fabsf(startPos2.y - endPos2.y) > FLT_EPSILON) &&
         (yi < fminf(startPos2.y, endPos2.y) ||
          (yi > fmaxf(startPos2.y, endPos2.y)))))
      collision = false;
...
bool CheckCollisionCircleLine(Vector2 center, float radius, Vector2 p1,
                              Vector2 p2) {
  float dx = p1.x - p2.x;
  float dy = p1.y - p2.y;
  ...
  float dotProduct = (((center.x - p1.x) * (p2.x - p1.x)) +
